...
const ThreeScene = () => {
  ...
  return <div ref={mountRef} className="fixed inset-0 -z-10 bg-black-100" />;
};
  ...
  if (topicId) {
      ...
      return (
        <div className="min-h-screen bg-black-50">
          ...
          <div className="relative z-10 max-w-4xl mx-auto pt-10 px-4">
            <Card className="p-6 bg-black shadow-md">
              <CardTitle className="text-black-800">Topik tidak ditemukan</CardTitle>
              <button 
    ...
    return (
      <div className="min-h-screen bg-black-100">
        <ThreeScene />
        <div className="relative z-10 max-w-4xl mx-auto pt-10 px-4">
          <Card className="mb-6 p-6 bg-black shadow-md">
            <CardTitle className="text-2xl mb-2 text-black-800">{topic.title}</CardTitle>
            <div className="text-sm text-black-600">
              Dibuat oleh {topic.author} • {topic.views} views • {topic.posts} posts
            </div>
          </Card>
          
          <Card className="p-6 mb-4 bg-black shadow-md">
            <div className="text-black-700">Isi diskusi akan muncul di sini</div>
          </Card>
  ...
  if (sub) {
    ...
    return (
      <div className="min-h-screen bg-black-100">
        ...
        <div className="relative z-10 max-w-4xl mx-auto pt-10 px-4">
          <Card className="mb-6 p-6 bg-black shadow-md">
            <CardTitle className="text-2xl mb-1 text-black-800">{decodedSub}</CardTitle>
          ...
          <div className="mb-4 flex justify-between items-center">
            <span className="font-semibold text-lg text-black-900">Topik</span>
            <button 
              className="bg-blue-600 hover:bg-blue-700 text-black px-4 py-2 rounded-lg text-sm font-medium transition-colors shadow-sm"
          ...
          <div className="flex flex-col gap-3">
            {topik.map((t) => (
              <div 
                key={t.id}
                className="w-full p-4 bg-black border border-black-200 rounded-lg hover:border-blue-300 hover:shadow-md cursor-pointer transition-all duration-200"
                onClick={() => router.push(`/komunitas?topicId=${t.id}&fromSub=${encodeURIComponent(sub)}`)}
              >
                <div className="flex justify-between items-start">
                  <div>
                    <div className="font-semibold text-black-900">{t.title}</div>
                    <div className="text-xs text-black-500">by {t.author}</div>
                  </div>
                  <div className="text-right text-xs text-black-500">
          ...
          <div className="flex flex-col lg:flex-row gap-8 w-full">
            <div className="w-full lg:w-80 lg:min-w-[20rem]">
              <Card className="p-6 sticky top-6">
                <CardTitle className="mb-6 text-xl text-black-900">📈 Topik Populer</CardTitle>
                <div className="flex flex-col gap-4">
                  {topikViews.map((topik, i) => (
                    <div 
                      key={topik.title} 
                      className="flex hover:text-black justify-between items-center p-3 rounded-lg hover:bg-blue-50 cursor-pointer transition-colors border-l-4 border-blue-100 hover:border-blue-400"
                    ...
                    >
                      ...
                      <span className="text-xs text-black-500 bg-black-100 px-2 py-1 rounded-full">
                        {topik.views} views
                      </span>
                    </div>
                  ))}
                  ...
                  <Card key={mat.title} className="p-6 h-full group hover:shadow-lg transition-all duration-300">
                    <div className="flex flex-col h-full">
                      <div className="flex items-center gap-3 mb-4">
                        ...
                        <CardTitle className="text-xl text-black-900 group-hover:text-blue-600 transition-colors">
                          {mat.title}
                        </CardTitle>
                      </div>
                      <p className="text-black-600 text-sm mb-6 leading-relaxed flex-grow">
                        {mat.desc}
                      </p>
